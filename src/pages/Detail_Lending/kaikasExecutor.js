...
const Toast = Swal.mixin({
  ...
  const checkOusdtBalance = async (address) => {
    ...
    const tokenContract = new window.caver.klay.Contract(balanceAbi, tokenContractAddress);
...
const allowanceChecker = async (address, activeWalletAddress) => {
    ...
    const tokenContract = new window.caver.klay.Contract(allowanceAbi, tokenContractAddress);

    const allowance = await tokenContract.methods.allowance(address, activeWalletAddress).call(); 
...
const approveExecutor = async (address, activeWalletAddress, amount) => {
...
const repayExecutor = async (address, activeWalletAddress, repayAmount, reclaimAmount) => {

    let transactionInfo = {}

    transactionInfo = await rePay(address, activeWalletAddress, repayAmount, reclaimAmount)
...
const emergencyRepayExecutor = async (account,activeWalletAddress) => {

    let transactionInfo = {}

    transactionInfo = await emergenctRepay(account,activeWalletAddress)
...
const lendAndBorrowExecutor = async (userAccount, activeWalletAddress, depositAmount, ltv) => {

    let transactionInfo = {}

    transactionInfo = await lendAndBorrow(userAccount, activeWalletAddress,depositAmount, ltv)
...
const lendAndBorrow = async (userAccount, activeWalletAddress, depositAmount, ltv) => {
...
const metamaskRepayTransaction = async (address, activeWalletAddress, repayAmount, reclaimAmount) => {
    ...
    const abi ={
      ...
      const amountBN = window.caver.utils.toPeb(reclaimAmount.toString(), 'KLAY');
...
const rePay = async (address, activeWalletAddress, repayAmount, reclaimAmount) => {
...
const metamaskExecutor = async (transactionInfo) => {
    if (typeof window.ethereum !== 'undefined') {
        ...
        provider
          ...
          .then(() => {
            executeMetamask(transactionInfo)
...
const emergenctRepay = async (address, activeWalletAddress) => {
...
async function executeMetamask (transactionInfo) {
...
const kaikasCreateWalletExecutor = async (accountAddress) => {

    const userAddress = accountAddress

    let transactionInfo = {}

    transactionInfo = await createActiveWallet(userAddress)
...
async function createActiveWallet (addr) {
